        ...
        app('validator')->extend('amountValue', function ($attribute, $value, $parameters, $validator) {
            ...
            if ($value != $valueBaseCalculated) {
                $validator->addReplacer(
                    'amountValue',
                    function ($message, $attribute, $rule, $parameters) use ($value, $valueBaseCalculated) {
                        return str_replace(
                            ':calculation',
                            $valueBaseCalculated,
                            str_replace(':value', $value, $message)
                        );
                    }
                );
                ...
                if (env('APP_DEBUG', false)) {
                    app('log')->debug(
                        "Validator amount_value fails! Received: $value, should be: "
                        .$valueBaseCalculated
                    );
                }
